...
- Для ускорения и повторного использования подсчитанных токенов GigaChat поддерживает `X-Session-ID`.
- В `AnalysisService` теперь передаётся `session_id` (если есть) и устанавливается в `gigachat.context.session_id_cvar` (при поддержке SDK).
- Передавайте `session_id` из вашего HTTP-эндпоинта (например, заголовок `X-Session-ID`) в вызовы анализатора.

Примеры запуска
- Быстрый интеграционный тест (локально использует `.env` переменные):

```powershell
& .\.venv\Scripts\python.exe tmp_run_gigachat_test.py
```
...
- DNS/NameResolutionError для `api.proxy.ai` означает, что ваша сеть не может резолвить адрес proxy — проверьте подключение/hosts/прокси.
- Временное отключение валидации SSL (`verify=False`) включено только для разработки; в production необходимо включить проверку сертификатов.
...
Тесты
- Можно добавить unit-тесты с моками для `gigachat` (рекомендую добавить проверку установки session_id в `gigachat.context`).
